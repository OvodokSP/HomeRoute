...
Если вам нужна инструкция уровня **«откройте это окно → введите эту команду → найдите такую строку»**, используйте [полный маршрут для новичка](beginner-guide.md).

## Текущий статус

Полностью автоматическая чистая установка пока **не выпущена**. Безопасный режим проверки уже готов; live backup/restore и rollback проверены на эталонной системе. Обычная команда, которая сама изменяет чистый роутер или VPS, намеренно заблокирована до первого clean-device reproduction (HL-502).
...
До успешного HL-502 обычная команда `apply` специально возвращает отказ и не вносит изменений. Для разработки HL-502 используется отдельная граница `reproduction-apply`; она не означает, что stable installer уже готов.
...
Если возникла ошибка или непонятный термин, сначала откройте [ЧаВо](faq.md).
