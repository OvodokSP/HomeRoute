...
Для первого HL-502 используется отдельный `reproduction-apply`, а не обычный stable `apply`.

`reproduction-apply` допускается только после закрытия HL-404, требует отдельного явного ACK, предназначен только для заранее согласованного тестового контура и обязан использовать те же backup/verify/rollback contracts, что и будущий stable installer. Он не означает, что обычный `apply` уже разрешён.

Обычный/stable `apply` остаётся BLOCKED до успешного HL-502. После clean reproduction результаты анализируются, и только затем пересматривается статус general live apply.
