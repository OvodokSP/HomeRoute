...
HOMEROUTE_SANDBOX_ROOT="$R" HOMEROUTE_TX_ID=router-1     sh "$ROUTER" sandbox-apply > "$BASE/router-first.out"
...
HOMEROUTE_SANDBOX_ROOT="$R" HOMEROUTE_TX_ID=router-2     sh "$ROUTER" sandbox-apply > "$BASE/router-second.out"
...
if HOMEROUTE_SANDBOX_ROOT="$RF" HOMEROUTE_TX_ID=router-fail    HOMEROUTE_SANDBOX_FORCE_VERIFY_FAIL=1    sh "$ROUTER" sandbox-apply > "$BASE/router-fail.out" 2>&1; then
...
N="$BASE/no-marker"
mkdir -p "$N"
if HOMEROUTE_SANDBOX_ROOT="$N" HOMEROUTE_TX_ID=no-marker    sh "$ROUTER" sandbox-apply > "$BASE/no-marker.out" 2>&1; then
